...
class ArticleController extends Controller
{
    ...
    public function index()
    {
        ...
        $articles = Cahce::remember('articles'.$page,3000,function(){
    ...
    public function show(Article $article)
    {
        ...
        $result = Cache::rememdebrForever('comment_article'.$article->id, function()use($article){
            $comments = Comment::where('article_id', $article->id)->where('accept', true)->get();
            $user  = User::findOrFail($article->user_id);
            return ['comments'=>$comments,'user'=>$user];
        });
        return view('article.show', ['article'=>$article, 'user'=>$result['user'], 'comments'=>$result['commets']]);
    }
